...
with tf.name_scope("rnn_weights"):
    with tf.name_scope("W_x"):
        Wx = tf.Variable(tf.truncated_normal([element_size, hidden_layer_size],                          mean=0,stddev=0.01))
        
    with tf.name_scope("W_h"):
        Wh = tf.Variable(tf.truncated_normal([hidden_layer_size,hidden_layer_size], mean=0, stddev=0.01))

    with tf.name_scope("bias"):
        b = tf.Variable(tf.truncated_normal([hidden_layer_size], mean=0, stddev=0.01))
...
initial_hidden = tf.truncated_normal([batch_size, hidden_layer_size], mean=0, stddev=0.01)
...
rnn_cell = tf.nn.rnn_cell.BasicLSTMCell(hidden_layer_size, forget_bias=1.0)
